...
- [数据采集层](#数据采集层)
  ...
  - [TikHubClient — 社交媒体 API](#tikhubclient--社交媒体-api)
...
│  ┌──────────┐  ┌──────────────┐  ┌────────────────┐               │
│  │WebScraper│  │TikHubClient  │  │GitHubCollector │               │
...
| **社交媒体** | TikHub API | 一个 API 覆盖 16+ 社交平台，无需自行处理各平台反爬机制 |
...
| 微博 | TikHub API | 关键词搜索：腾讯WorkBuddy |
| 小红书 | TikHub API | 关键词搜索：WorkBuddy |
| 知乎 | TikHub API | 关键词搜索：腾讯WorkBuddy |
| B站 | TikHub API | 关键词搜索：WorkBuddy 腾讯 |
...
| X / Twitter | TikHub API | 关键词搜索：Anthropic Claude |
| YouTube | TikHub API | 关键词搜索：Claude AI Anthropic |
| Reddit | TikHub API | 关键词搜索 + 子版块：r/ClaudeAI |
| Instagram | TikHub API | 关键词搜索：Anthropic Claude |
...
### TikHubClient — 社交媒体 API

**文件：** `collectors/tikhub_client.py`

**适用场景：** 微博、小红书、B站、知乎、X/Twitter、YouTube、Reddit、Instagram 等有反爬机制的社交平台。

**TikHub API 概况：**
- Base URL：`https://api.tikhub.io`（中国大陆使用 `https://api.tikhub.dev`）
- 认证方式：`Authorization: Bearer <TOKEN>`
- 覆盖平台：16+，端点 1000+
- 速率限制：10 QPS
- 文档：[Swagger UI](https://api.tikhub.io) | [Apifox 文档](https://docs.tikhub.io)

**已对接的端点：**

| 平台 | 搜索端点 | 关键参数 |
|------|---------|---------|
| 微博 | `/api/v1/weibo/search_by_keyword` | `keyword` |
| 小红书 | `/api/v1/xiaohongshu/app_v2/search_by_keyword` | `keyword` |
| B站 | `/api/v1/bilibili/web/search_by_keyword` | `keyword` |
| 知乎 | `/api/v1/zhihu/search_by_keyword` | `keyword` |
| X/Twitter | `/api/v1/twitter/web/search_by_keyword` | `keyword` |
| YouTube | `/api/v1/youtube/web/search_by_keyword` | `keyword` |
| Reddit | `/api/v1/reddit/web/search_by_keyword` | `keyword`, `subreddit` |
| Instagram | `/api/v1/instagram/web/search_by_keyword` | `keyword` |

**数据标准化：** 各平台返回的 JSON 结构各异，`TikHubClient` 通过 `_parse_single_record()` 方法将不同平台的字段映射为统一的 `RawItem` 模型，包括 URL 拼接、作者提取、时间解析等。

**限流与重试：** 内置 `asyncio.Semaphore(10)` 实现 QPS 限流，`@retry` 装饰器实现最多 3 次指数退避重试。
